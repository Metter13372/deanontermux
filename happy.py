...
█▀█ █▀█ █▀▀ █▀▀  █ 
                            ...
                            Welcome to Freiz TOOL, Press "ENTER" to continue !'''), pystyle.Colors.purple_to_red, pystyle.Colorate.Vertical, enter=True)
...
РАЗРАБОТЧИК: @Fr31zep \n'''), pystyle.Colors.green, interval = 0.001)
...
while True:
    ...
    select = pystyle.Write.Input("\n\n[?] Выберите пункт меню: ", pystyle.Colors.green, interval = 0.001)
...
"""
    ...
    if select == "7":
        ...
        def get_proxy():
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    pystyle.Write.Print(f"\nПроизошла ошибка -> {response.status_code}", pystyle.Colors.green, interval=0.005)

            except Exception as e:

                pystyle.Write.Print(f"\nПроизошла ошибка -> {str(e)}", pystyle.Colors.green, interval=0.005)
        ...
        if proxies:
            ...
            for proxy in proxies:
                ...
                pystyle.Write.Print("\n" + proxy, pystyle.Colors.green, interval=0.005)

            print()

        else:

            pystyle.Write.Print("Прокси недоступно.", pystyle.Colors.green, interval=0.005)
    ...
    if select == "9":
        ...
        password_length = int(
            ...
            pystyle.Write.Input("[?] Введите длину пароля -> ", pystyle.Colors.green, interval=0.005)

        )

        complexity = pystyle.Write.Input(

            "[?] Выберите сложность (low, medium, high): ", pystyle.Colors.green, interval=0.005)
...
Отправили письмо - следуйте этим же шагам в правильной последовательности. Выйдите из Tor, Linux, а затем из VPN.\n''', pystyle.Colors.green, interval = 0.005)
    ...
    if select == '13':
        ...
        pystyle.Write.Print("\n[?] Выберите режим: ", pystyle.Colors.green, interval=0.005)

        pystyle.Write.Print("\n\n[?] 1 - проверить часто используемые порты", pystyle.Colors.green, interval=0.005)

        pystyle.Write.Print("\n\n[?] 2 - проверить указанный порт", pystyle.Colors.green, interval=0.005)

        mode = pystyle.Write.Input("\n\n[?] Ваш выбор: ", pystyle.Colors.green, interval=0.005)
        ...
        if mode == "1":
            ...
            for port in ports:
                ...
                if result == 0:
                    ...
                    pystyle.Write.Print(f"[+] Порт {port} открыт", pystyle.Colors.green, interval=0.005)

                else:

                    pystyle.Write.Print(f"[+] Порт {port} закрыт", pystyle.Colors.green, interval=0.005)
            ...
            port = pystyle.Write.Input("\n[?] Введите номер порта: ", pystyle.Colors.green, interval=0.005)
            ...
            pystyle.Write.Print("\n[!] Неизвестный режим", pystyle.Colors.green, interval=0.005)
    ...
    if select == "15":
        ...
        start_url = pystyle.Write.Input("[?] Введите ссылку -> ", pystyle.Colors.green, interval=0.005)
        ...
        def dos1():
            ...
            try:
                ...
                def make_request(url):
                    ...
                    print(f"\n{colorama.Fore.green}[{colorama.Fore.WHITE}+{colorama.Fore.green}]{colorama.Fore.MAGENTA} Атака началась, состояние сайта: ", response.status_code)

                def dos():

                    url = input(f"{colorama.Fore.green}[{colorama.Fore.WHITE}?{colorama.Fore.green}]{colorama.Fore.MAGENTA} Введите ссылку : ")

                    power = input(f"{colorama.Fore.green}[{colorama.Fore.WHITE}?{colorama.Fore.green}]{colorama.Fore.MAGENTA} Выберите режим (1 - Слабый/2 - Средний/3 - Мощный) : ")
                    ...
                    if power == "1":
                        ...
                        print(f"\n{colorama.Fore.green}[{colorama.Fore.WHITE}!{colorama.Fore.green}]{colorama.Fore.MAGENTA} Нет такого режима!")

                dos()

            except:

                print(f'\n{colorama.Fore.green}[{colorama.Fore.WHITE}!{colorama.Fore.green}]{colorama.Fore.MAGENTA} Произошла ошибка! Проверьте ввод данных!')
    ...
    if select == "17":
        ...
        pystyle.Write.Print("\n[+] Страна: " + card["Страна"], pystyle.Colors.green, interval=0.005)

        pystyle.Write.Print("\n[+] Номер карты: " + card["Номер карты"], pystyle.Colors.green, interval=0.005)

        pystyle.Write.Print("\n[+] Срок действия: " + card["Срок действия"], pystyle.Colors.green, interval=0.005)
    ...
    if select == "18":
        ...
        mac_address = pystyle.Write.Input("[?] Введите Mac-Address -> ", pystyle.Colors.green, interval = 0.005)  # Replace this with the MAC address you want to lookup

        vendor = mac_lookup(mac_address)

        pystyle.Write.Print(f"Vendor: {vendor}", pystyle.Colors.green, interval = 0.005)

    if select == "19":

        pystyle.Write.Print('''\nПровоцируем жертву на угрозы, оски, буллинг и тд. Дальше пишем на почту DMCA@telegram.org. В письме указывем скриншоты булинга, угроз и тд. "Здраствуйте, хочу подать жалобу на (юзер и айди типа).  уважением, пользователь" и все, главное скриншоты - доказательства . А теперь знай - двинулся . гей ( у создателя купон )\n''', pystyle.Colors.green, interval = 0.005)

    if select == "20":

        num_agents = pystyle.Write.Input("\n[?] Введите кол-во User-agent -> ", pystyle.Colors.green, interval = 0.005)
...
"Mozilla/5.0 (X11; Linux x86_64; rv:{0}.0) Gecko/{0}{1:02d} Firefox/{0}.0",
            ...
            for _ in range(num_agents):
                ...
                pystyle.Write.Print("\n" + user_agent, pystyle.Colors.green, interval = 0.005)
    ...
    if select == "21":
        ...
        pystyle.Write.Print("\n[+] Поддержать проект: https://yoomoney.ru/to/4100118108013896 \n\n[+] ТГК -> https://t.me/FreizerSOFTS \n[+] ИНФОРМАЦИЯ: ЛУЧШИЙ БЕСПЛАТНЫЙ МУЛЬТИ-ИНСТРУМЕНТ ДЛЯ ИНФОРМАЦИОННОЙ БЕЗОПАСНОСТИ. ЗА ВАШИ ДЕЙСТВИЯ ОТВЕТСТВЕННОСТЬ НЕСЁТЕ ИСКЛЮЧИТЕЛЬНО ВЫ \n\n[+] ДАТА СОЗДАНИЯ СОФТА -> 22.04.2024 \n\n[!] СПАСИБО , ЧТО ЦЕНИТЕ СТАРАНИЯ\n", pystyle.Colors.green, interval = 0.005)
